 ...
 *
 * Any files named "directories.yaml" in the input tree will be parsed, and
 ...
 *
 *
 * Any files named "files.yaml" in the input tree will be parsed, and
 * corresponding files will be created in the output tree at the same
 * directory level as the files.yaml
 * Format:
 * - file_a:
 *   mode: 0600
 *   owner: root
 *   group: root
 * - file_b
 * NOTE THAT EXISTING FILES WILL NOT BE OVERWRITTEN. This allows for deploying
 * blank files to the appdata directory, without modifying a file if it already
 * exists. This is good for deploying files to persistent or logs, if they are
 * required to exist for the application to run, but we don't want to overwrite
 * content generated by the application
 * File permissions and owners of existing files will be changed if necessary.
 *
 *
...
walker.on("file", function(root_path, stat, next) {
	...
	} else if (rel_file.match(/^files.ya?ml$/)) {
		console.log("Creating files in: " + rel_path);
		processFilesYaml(input_path + rel_path, output_path + rel_dir);
	} else {
...
function applyFilePermissions(options, path){
	if(options.mode  != null){ execSync('chmod ' + options.mode  + ' ' + path); }
	if(options.owner != null){ execSync('chown ' + options.owner + ' ' + path); }
	if(options.group != null){ execSync('chgrp ' + options.group + ' ' + path); }
}
...
/**
 * Processes a files.yaml file in order to create a set of empty
 * files in the directory
 */
function processFilesYaml(input_file, output_root_dir){
	let files = yaml.readSync(input_file);

	if(!Array.isArray(files)){
		console.error("Expected root object to be an array in: '" + input_file + "', got: ");
		console.dir(files);
		process.exit(1);
	}
	...
	for(let file of files){
		...
		if(typeof file == 'string'){
			...
			if(keys.length !== 1){
				console.error("Expected single file name to map to file options, got: ");
				console.dir(file);
				process.exit(1);
			}
			file_name = keys[0];
			file_opts = file[file_name];
		}

		if(!fs.existsSync(output_root_dir + file_name)){
			fs.writeFileSync(output_root_dir + file_name, '');
		}

		applyFilePermissions(file_opts, output_root_dir + file_name);
	}
}
